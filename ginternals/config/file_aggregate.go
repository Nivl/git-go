...
// Objectformat returns the hash algorithm used by the repository.
func (cfg *FileAggregate) Objectformat() (name string, ok bool) {
	source := cfg.global
	if cfg.local.Section("extensions").HasKey("objectformat") {
		source = cfg.local
	}

	v := source.Section("extensions").Key("objectformat").String()
	if v == "" {
		return "", false
	}
	return v, true
...
// UpdateObjectformat updates the object format.
func (cfg *FileAggregate) UpdateObjectformat(objectFormat string) {
	cfg.local.Section("extensions").Key("objectformat").SetValue(objectFormat)
}
